...
def mejoraMaxBloques_Paralelo(funcion, variables, vector, tol):
    ...
    while (error> tol):
        ...
        results = [pool.apply_async(calcular_nuevoX,
                                    args=(i, variables, funcion, vector, solvers))
                   for i in range(0, len(variables))]
        soluciones = []
        pool.close()
        for i in range(0, len(variables)):
...
def resolverParalelo(N,tol):
    ...
    print(datetime.datetime.now().time())
    resultado = mejoraMaxBloques_Paralelo(funcion, variablesMatriz, valoresparaeval, tol)
...
SOLO se puede ejecutar en este IF
"""
if __name__ == '__main__':
    print(resolverParalelo(50,0.01))
